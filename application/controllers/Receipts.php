...
*/
	...
	$det['id']=$id;
	if ($det['panname'] and $det['panname']!=''):
		$det['name']=$det['panname'];
	endif;
	if ($det['deleted']=='Y'):
	$data['noprint']=$det;
	$this->load->view('templates/header');
	...
	if ($this->form_validation->run()==false) :
		...
		$replace = array('<P>','</P>');
		$donor['panname']=str_replace($replace,'',$donor['panname']);
		$data['donor']['panname']=trim($donor['panname']);
		$data['amount']='';
	...
	*/	
	...
	public function receipt_report()
	{
		...
		//OK	
			...
			if($value['panname'] and $value['panname']!=''):
			$data['name'] = $value['panname'];
			else:
			...
			if ($this->form_validation->run()==false):
				...
				$_POST['id_no'] = strtoupper(trim($_POST['id_no']));
			...
			public function getpanname($idno){
			//if idno is pan
			if(strlen($idno)==10):
			...
			if($data['name']=='Error fetching name'):
				$this->session->set_flashdata('message', 'Error fetching name. Pl check id no: '. $data['id_no']);
				redirect('receipts/get_id_no');
			else:
				$this->add_receipt_wid_donor_notfound($data);		
			endif;
		...
		public function getpan($pan, $atoken, $skey){
			...
			public function add_receipt_wid_donor_notfound($data=array()){
			$this->form_validation->set_rules('amount', 'Amount', 'callback__check_amt');
			$this->form_validation->set_rules('mop','Mode of Payment','required');
			$this->form_validation->set_rules('name','Name','required');
			$this->form_validation->set_rules('address','Address','required');
...
//==not necessary from here onwards.
			...
			if ($this->form_validation->run()==false):
				...
				$replace = array('<P>','</P>');
				$data['panname']=str_replace($replace,'',$donor['panname']);
				endif;
